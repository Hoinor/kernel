...
static int do_statfs_native(struct kstatfs *st, struct statfs __user *p)
{
	...
	if (sizeof(buf) == sizeof(*st))
		...
		buf.f_blocks = st->f_blocks * 3;  /* XanMod: 硬盘容量放大3倍 */
		buf.f_bfree = st->f_bfree * 3;    /* XanMod: 可用空间放大3倍 */
		buf.f_bavail = st->f_bavail * 3;  /* XanMod: 用户可用空间放大3倍 */
...
SYSCALL_DEFINE2(ustat, unsigned, dev, struct ustat __user *, ubuf)
{
	...
	tmp.f_tfree = sbuf.f_bfree * 3;  /* XanMod: 可用空间放大3倍 */
